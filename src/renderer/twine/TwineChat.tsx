...
import {
  ...
  type TwineDocumentContextScope,
  type TwinePageState,
...
export interface TwineChatProps extends InternalPageProps {
...
export function TwineChat({
  ...
  const [documentProposal, setDocumentProposal] =
    useState<TwineDocumentProposal>();
  const documentApprovalResolver = useRef<
  ...
  const requestDocumentApproval = useCallback(
    ({ proposal }: TwineDocumentAgentApprovalRequest): Promise<boolean> => {
      documentApprovalResolver.current?.(false);
      setDocumentProposal(proposal);
      return new Promise((resolve) => {
        documentApprovalResolver.current = resolve;
      });
    },
    [],
  );

  const resolveDocumentApproval = useCallback((approved: boolean): void => {
    const resolve = documentApprovalResolver.current;
    documentApprovalResolver.current = undefined;
    setDocumentProposal(undefined);
    resolve?.(approved);
  }, []);
  ...
  useEffect(() => {
    if (!documentAgent || !pageState.data.documentContextEnabled) {
      runtime.setDocumentToolHandler(undefined);
      return;
    }
    runtime.setDocumentToolHandler((call, requestId, isActive) =>
      documentAgent.execute(call, {
        approvalMode: pageState.data.approvalMode,
        historyGroup: `twine:${requestId}`,
        scope: pageState.data.documentContextScope,
        requestApproval: requestDocumentApproval,
        shouldContinue: isActive,
      }),
    );
    return () => runtime.setDocumentToolHandler(undefined);
  }, [
    documentAgent,
    pageState.data.approvalMode,
    pageState.data.documentContextEnabled,
    pageState.data.documentContextScope,
  ...
  function setDocumentContext(
    ...
    setNotice(
      documentContextEnabled
        ? translate('twine.documentConsentNotice')
        : undefined,
    );
  }
  ...
  async function startGeneration(
    ...
    try {
      await getFlyoffApi().startTwineGeneration?.({
        approvalMode: pageState.data.approvalMode,
        ...(pageState.data.documentContextEnabled &&
        documentAgent?.available
          ? {
              documentAgent: {
                enabled: true as const,
                scope: pageState.data.documentContextScope,
                ...(documentAgent.currentTarget
                  ? {
                      currentTarget: documentAgent.currentTarget,
                    }
                  : {}),
              },
            }
    ...
    <main className="twine-page" aria-label={translate('twine.conversation')}>
      ...
      >
        ...
        <TwineComposer
          ...
          approvalMode={pageState.data.approvalMode}
          documentAgentAvailable={documentAgent?.available ?? false}
          documentCurrentAvailable={Boolean(
            documentAgent?.currentTarget,
          )}
          documentContextEnabled={pageState.data.documentContextEnabled}
          documentContextScope={pageState.data.documentContextScope}
          draft={draft}
      ...
      {documentProposal ? (
        <Dialog
          closeLabel={translate('windowControls.close')}
          description={documentProposal.description}
          footerEnd={
            <button
              className="flyoff-dialog__button--primary"
              onClick={() => resolveDocumentApproval(true)}
              type="button"
            >
              {translate('twine.diagramApply')}
            </button>
          }
          footerStart={
            <button
              data-dialog-initial-focus
              onClick={() => resolveDocumentApproval(false)}
              type="button"
            >
              {translate('twine.diagramReject')}
            </button>
          }
          onCancel={() => resolveDocumentApproval(false)}
          size="standard"
          title={
            documentProposal.kind === 'diagram'
              ? translate('twine.diagramProposalTitle')
              : translate('twine.noteProposalTitle')
          }
        >
          <div className="twine-document-proposal">
            {documentProposal.kind === 'diagram' ? (
              <dl>
                <div>
                  <dt>{translate('twine.diagramType')}</dt>
                  <dd>{documentProposal.diagramType}</dd>
                </div>
                <div>
                  <dt>{translate('twine.diagramChanges')}</dt>
                  <dd>
                    {documentProposal.operationSummary.addedElements +
                      documentProposal.operationSummary.updatedElements +
                      documentProposal.operationSummary.removedElements +
                      documentProposal.operationSummary.addedRelationships +
                      documentProposal.operationSummary.updatedRelationships +
                      documentProposal.operationSummary.removedRelationships}
                  </dd>
                </div>
                <div>
                  <dt>{translate('twine.diagramDiagnostics')}</dt>
                  <dd>{documentProposal.diagnosticsAfter.length}</dd>
                </div>
              </dl>
            ) : (
              <dl>
                <div>
                  <dt>{translate('twine.noteChanges')}</dt>
                  <dd>{documentProposal.operationSummary.operationCount}</dd>
                </div>
              </dl>
            )}
            {documentProposal.operationSummary.destructive ? (
              <p role="alert">
                {documentProposal.kind === 'diagram'
                  ? translate('twine.diagramDestructiveWarning')
                  : translate('twine.noteDestructiveWarning')}
              </p>
            ) : null}
          </div>
        </Dialog>
