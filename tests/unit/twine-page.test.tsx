...
import type { TwineDocumentAgent } from '../../src/renderer/twine';
import {
  INTERNAL_PAGE_IDS,
  type FlyoffApi,
...
describe('Twine chat page', () => {
  ...
  it('requires explicit document consent and confirms an agent proposal', async () => {
    let generationListener:
      | ((event: TwineGenerationEvent) => void)
      | undefined;
    ...
    const documentAgent: TwineDocumentAgent = {
      available: true,
      currentTarget: {
        nodeId: 'diagram-1',
        pageType: 'diagram',
      },
      execute: vi.fn(async (_call, options) => {
        const approved = await options.requestApproval({ proposal });
        return approved
          ? { ok: true, text: '{"status":"applied"}' }
          : { ok: false, text: 'rejected' };
      }),
    };
    const { onStateChange } = renderTwine(undefined, documentAgent);
    await waitFor(() =>
      expect(getTwineCredentialStatus).toHaveBeenCalledOnce(),
    );
    ...
    fireEvent.change(screen.getByRole('textbox', { name: 'Message Twine' }), {
      target: { value: 'Rename the class' },
    });
    ...
    expect(request.documentAgent).toEqual({
      enabled: true,
      scope: 'current',
      currentTarget: {
        nodeId: 'diagram-1',
    ...
    expect(
      await screen.findByRole('dialog', { name: 'Review UML proposal' }),
    ).toBeTruthy();
    fireEvent.click(
  ...
  it('reviews note changes through the same document agent surface', async () => {
    let generationListener:
      | ((event: TwineGenerationEvent) => void)
      | undefined;
    const startTwineGeneration = vi.fn().mockResolvedValue(undefined);
    ...
    renderTwine(undefined, documentAgent);
    await waitFor(() =>
      expect(getTwineCredentialStatus).toHaveBeenCalledOnce(),
    );
